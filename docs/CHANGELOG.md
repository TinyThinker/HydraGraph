...
- **The hub at the apex is built, in its own repo** (2026-09-21). Phase C of the launch
  ...
  **It deploys as an assets-only Worker, not Pages** — Pages is the legacy flow and the
  dashboard now routes new projects through Workers with static assets, whose wizard
  prefills `npx wrangler deploy` against a wrangler config the repo didn't yet have.
  `wrangler.jsonc` omits `main` deliberately: Cloudflare does not apply `_headers` to
  responses generated by Worker code, so having no Worker code is what keeps the CSP on
  every response — re-verified against `wrangler dev`, which serves assets the way
  production does, with all four headers present on the HTML *and* the SVG and unknown
  paths returning a real 404. The app itself stays on Pages; a working deploy had no
  reason to migrate. **Not live yet:** creating the Worker and attaching
  `tinythinkerlabs.dev` is the user's step, and Phase C's "done when" is not met until
  it lands.
